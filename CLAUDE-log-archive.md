# PackTimes — older version log

Split out of `CLAUDE.md` on 2 September 2026. Nothing edited, order preserved (newest first).
**Not read by default.** Open it when you need a version that is no longer in CLAUDE.md's recent log.

`trim_log.py` prepends new entries here as they drop out of the main file.
...
**(It bit. Resolved in v374 — Open-Meteo's `utc_offset_seconds` became the lookup, with the
device-clock-wins-within-3h rule protecting the rider-on-route case this note worried about.)**
