...
- **Verified:** `node verify.js`; classifier run on live Overpass (Melbourne, Bright, Engelberg). Live corridor search on the demo route: 43 water, 8 vending, 5 train, 6 bike, 1 outdoor, 29 wc. Tiles, legend and sheet boxes checked in the preview. Funicular stations and bubbler-tagged springs fixed after that run. Shower attach checked on real Muttaburra QLD data: the hot shower lands on the toilet block, the camp's own shower=yes tag shows on its tile, and a beach shower with nothing nearby is dropped. Demo stops restored. Backup `backup/index-v382-pre-v383.html`. NOT pushed.
